...
void Dijkstra::set_dot(string dot_symbol)
{
	...
	do
	{
		...
		if (red < 0 || stupac < 0 || red > columns || stupac > rows)
		{
			clearScreen;
			cout << "Error" << endl
	...
	} while (red<0 || stupac<0 || red>columns || stupac>rows);
...
int Dijkstra::get_distance(zeZidom current, zeZidom next)
{
...
void Dijkstra::scatch(zeZidom location)
{
	...
	while (current.x != B.x || current.y != B.y)
	{
		...
		for (int x = Start_X; x <= End_X; x++)
		{
			for (int y = Start_Y; y <= End_Y; y++)
			{
				...
				else if (x < 0 || x >= columns || y < 0 || y >= rows) continue; //must be in boundaries
